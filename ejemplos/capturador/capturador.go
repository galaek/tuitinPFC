...
var (
	...
	search         *string = flag.String("search", "#HableConEllas", "keywords to search for, comma delimtted")
	users          *string = flag.String("users", "", "list of twitter userids to filter for, comma delimtted")
	collection     *string = flag.String("collection", "prueba1", "name of the collection where we store the Tweets")
)
...
func main() {
	...
	c := session.DB("reprobando").C(collectionName)
