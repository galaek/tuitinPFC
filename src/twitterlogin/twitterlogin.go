	...
	if err != nil {
		...
		if err != nil {
			...
			if err != nil {
				...
				fmt.Fprintf(rw, "Sesión iniciada como " + data.ScreenName + "</br>")
				fmt.Fprintf(rw, "UserID =  " + strconv.FormatInt(data.UserID, 10) + "</br>")
				fmt.Fprintf(rw, "AccessKey " + data.AccessKey + "</br>")
				fmt.Fprintf(rw, "SecretKey " + data.SecretKey + "</br>")
				...
				// }
				fmt.Fprintf(rw, "<a href=\"/formtest\">Go take a survey!</a></br>")
...
func TimelinesHandler(rw http.ResponseWriter, req *http.Request) {
	...
	if err != nil {
		...
		if err != nil {
			...
			http.SetCookie(rw, SessionUpdateCookie(sessionID))
			t, _ := template.New("encabezado").Parse("Let's read some timelines!</br>")
			_ = t.ExecuteTemplate(rw, "encabezado", nil)
			//rw.Header().Set("Content-Type", "text/html;charset=utf-8")
			fmt.Fprintf(rw, "<a href=\"/\">Back to main</a></br>")
...
func FormTestHandler(rw http.ResponseWriter, req *http.Request) {
	var (
	//	url       string
		err       error
		sessionID string
	...
	if err != nil {
		...
		if err != nil {
			...
			http.SetCookie(rw, SessionUpdateCookie(sessionID))
				if tmpl, err := template.ParseFiles("templates/test2.html"); err != nil {
					http.Error(rw, err.Error(), http.StatusInternalServerError)
				} else {
					tmpl.Execute(rw, nil)
				}
...
func FormResultsHandler(rw http.ResponseWriter, req *http.Request) {
	var (
	//	url       string
		err       error
		sessionID string
	...
	//rw.Header().Set("Content-Type", "text/html;charset=utf-8")
	//fmt.Println(req.Cookies())
	result.Nombre = req.FormValue("nombre")
	result.Pais = req.FormValue("pais")
	result.Ciudad = req.FormValue("ciudad")
	result.Genero = req.FormValue("sex")
	sessionID, err = GetSessionID(req)
	if err != nil {
		fmt.Fprintf(rw, "Debes loguearte con Twitter para poder hacer la super mega encuesta</br>")
		fmt.Fprintf(rw, "<a href=\"/\">Back to main</a></br>")
	...
	if err != nil {
		...
		if err != nil {
			...
			http.SetCookie(rw, SessionUpdateCookie(sessionID))
			if tmpl, err := template.ParseFiles("templates/results.html"); err != nil {
				http.Error(rw, err.Error(), http.StatusInternalServerError)
			} else {
				tmpl.Execute(rw, nil)
			}
			...
			//rw.Header().Set("Content-Type", "text/html;charset=utf-8")
			fmt.Fprintf(rw, "<a href=\"/\">Back to main</a></br>")
			fmt.Fprintf(rw, "Datos leidos:</br>")
			fmt.Fprintf(rw, "Nombre: " + result.Nombre + "</br>")
			fmt.Fprintf(rw, "Pais: " + result.Pais + "</br>")
			fmt.Fprintf(rw, "Ciudad: " + result.Ciudad + "</br>")
			fmt.Fprintf(rw, "Genero: " + result.Genero + "</br>")
			//go GetUserTimeline(sessionID)
			//fmt.Fprintf(rw, "Proceso de captura de datos lanzado")
		}
	}
...
func init() {
	...
	http.Handle("/formtest/", &authHandler{handler: FormTestHandler})
	http.Handle("/formresults/", &authHandler{handler: FormResultsHandler})
